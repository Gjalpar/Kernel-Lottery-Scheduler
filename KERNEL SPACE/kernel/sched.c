...
struct user_struct1 {
    int					id;
    int					num_of_process;
    struct user_struct1	*next;
};

void	free_user_struct1(struct user_struct1 *node) {
    struct user_struct1	*tmp;
    while (node) {
        tmp = node;
        node = node->next;
        kfree(tmp);
...
struct user_struct1	*search(struct user_struct1 *node, int id) {
    while (node) {
        if (node->id == id)
            return node;
        node = node->next;
...
void	add(struct user_struct1 **node, int id) {
    struct user_struct1	*new_element = kmalloc(sizeof(struct user_struct1), GFP_KERNEL);

    new_element->id = id;
    new_element->num_of_process = 1;
    new_element->next = NULL;
    if (!*node) {
        *node = new_element;
        return ;
    }
    struct user_struct1	*tmp = *node;
    while (tmp->next)
        tmp = tmp->next;
...
// PHASE 2 SPRING 2024 //
    if (schedule_flag == 0) {
        ...
        int total_ticket = 0;
        struct user_struct1	*user_list = NULL, *tmp2;

        list_for_each(tmp, &runqueue_head) {
            p = list_entry(tmp, struct task_struct, run_list);
            tmp2 = search(user_list, p->uid);
            if (!tmp2)
                add(&user_list, p->uid);
            else
                tmp2->num_of_process++;
        ...
        list_for_each(tmp, &runqueue_head) {
            ...
            tmp2 = search(user_list, p->uid);
            if (tmp2->id <= 1000)
                p->ticketno = USER_PER_TICKETS;
            else
                p->ticketno = USER_PER_TICKETS / tmp2->num_of_process;
            total_ticket += p->ticketno;
        }
        free_user_struct1(user_list);

        if (total_ticket) {
            unsigned int random_ticket;
            get_random_bytes(&random_ticket, sizeof(random_ticket));
            random_ticket %= total_ticket;

            // Select the process based on the random ticket
            list_for_each(tmp, &runqueue_head) {
                p = list_entry(tmp, struct task_struct, run_list);
                if (random_ticket < p->ticketno) {
                    if (can_schedule(p, this_cpu)) {
                        next = p;
                        break;
                    }
